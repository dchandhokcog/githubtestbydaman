...
namespace PayPal.Payments.SampleStore.CS.Handler
{
	...
	/// </remarks>
	/// -----------------------------------------------------------------------------
	public class FraudReviewHandler : BaseTransactionHandler
	{
		...
		public override Hashtable SubmitTransaction(Hashtable RequestData)
		{
			try
			{
				...
				int mCount;
				string mRequestId;
				FraudReviewTransaction FraudReviewTrans;
				...
				FraudReviewTrans = new FraudReviewTransaction(RequestData[PayPal.Payments.SampleStore.CS.Common.SampleStoreConstants.FIELD_ORIGID].ToString(), RequestData[PayPal.Payments.SampleStore.CS.Common.SampleStoreConstants.FIELD_UPDATEACTION].ToString(), UserInfo, PayflowConnectionData, mRequestId);
				
				for (mCount = 0; mCount <= ExtDataList.Count - 1; mCount++)
				{
					FraudReviewTrans.AddToExtendData((ExtendData)(ExtDataList[mCount]));
				}
				...
				{
					FraudReviewTrans.Verbosity = RequestData[PayPal.Payments.SampleStore.CS.Common.SampleStoreConstants.FIELD_VERBOSITY].ToString();
				}
