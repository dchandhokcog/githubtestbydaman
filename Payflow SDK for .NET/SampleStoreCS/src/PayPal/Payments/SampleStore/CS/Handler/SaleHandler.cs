...
namespace PayPal.Payments.SampleStore.CS.Handler
{
	...
	public class SaleHandler : BaseTransactionHandler
	{
		/// <summary>
		...
		public override Hashtable SubmitTransaction(Hashtable RequestData)
		{
			try
			{
				...
				SaleTrans = new SaleTransaction(UserInfo, PayflowConnectionData, Invoice, Tender, mRequestId);
				
				for (mCount = 0; mCount <= ExtDataList.Count - 1; mCount++)
				{
					SaleTrans.AddToExtendData((ExtendData)(ExtDataList[mCount]));
				}
				...
				{
					SaleTrans.Verbosity = RequestData[PayPal.Payments.SampleStore.CS.Common.SampleStoreConstants.FIELD_VERBOSITY].ToString();
				}
				...
				{
					SaleTrans.OrigId = RequestData[PayPal.Payments.SampleStore.CS.Common.SampleStoreConstants.FIELD_ORIGID].ToString();
				}
